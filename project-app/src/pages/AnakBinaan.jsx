...
export default function AnakBinaan() {
    ...
    const [query, setQuery] = useState(""); // state untuk search
    ...
    // ✅ useEffect dengan dependency [query] + debounce 500ms
    useEffect(() => {
        setLoading(true);
        ...
        return () => clearTimeout(timeout); // cleanup: batalkan timeout sebelumnya
    }, [query]); // dipanggil ulang setiap query berubah
    ...
    const handleSubmit = () => {
        axios
            ...
            .then(() => {
                ...
                // reset query supaya useEffect terpanggil ulang dan data ter-refresh
                setQuery("");
            })
            .catch((error) => {
    ...
    return (
        <div className="p-6">
            ...
            {showForm && (
                <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs font-semibold text-gray-500 mb-1 block">Nama Anak</label>
                            <input
                                type="text"
                                name="nama_anak"
                        ...
                        <div>
                            <label className="text-xs font-semibold text-gray-500 mb-1 block">Tanggal Masuk</label>
                            <input
                                type="date"
                                name="tanggal_masuk"
                        ...
                        <div>
                            <label className="text-xs font-semibold text-gray-500 mb-1 block">ID Pembina</label>
                            <input
                                type="number"
                                name="id_pembina"
            ...
            <div className="relative mb-4">
                ...
                />
                {/* Spinner muncul di dalam search bar saat loading */}
                {loading && (
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-[#293040] text-[#D3AC2B]">
                            ...
                            <th className="text-left px-5 py-3.5 font-semibold">Tanggal Lahir</th>
                        ...
                        {loading ? (
                            ...
                            anakBinaans.map((anak, index) => (
                                ...
                                >
                                    ...
                                    <td className="px-5 py-3.5 text-gray-600">{anak.tanggal_lahir}</td>
                                    <td className="px-5 py-3.5 text-gray-600">{anak.tanggal_masuk}</td>
                                    <td className="px-5 py-3.5 text-gray-600">{anak.id_pembina}</td>
                                    <td className="px-5 py-3.5 text-gray-600 max-w-[200px] truncate">{anak.alamat}</td>
                                </tr>
                            ))
                        )}
                    </tbody>
